...
def evaluate_university_row(row, scores):
    ...
    student_score = calculate_wheighted_score(student_values, weights)
...
def check_weak_subject(row, scores):
    comparisons = [
        ...
        (scores["탐구1과목"] or "탐구1", scores["탐구1"], row.get("탐구1_70")),
        (scores["탐구2과목"] or "탐구2", scores["탐구2"], row.get("탐구2_70"))
    ]
    warnings = []
...
class UniversityRecommendationGUI:
    ...
    def create_widgets(self):
        ...
        ttk.Label(input_frame, text="수학백분위").grid(row=0, column=2, padx=5, pady=5)
        ...
        ttk.Combobox(input_frame,
                     ...
                     values=INQUIRY_SUBJECT,
                     width=14).grid(row=1, column=5, padx=5, pady=5)
        ...
        ttk.Combobox(input_frame,
                     ...
                     width=12, state="readonly").grid(row=2, column=4, padx=5, pady=5)

        ttk.Button(input_frame, text="대학추천하기", command=self.run_recommendation).grid(
            row=2,
            column=5,
            padx=5,
            pady=5,
        ...
        ttk.Button(input_frame, text="결과를 EXCEL에 저장", command=self.save_results).grid(
            row=2,
            column=7,
            padx=5,
            pady=5,
